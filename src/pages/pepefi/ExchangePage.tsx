...
import { ASSET_META, ASSETS_LIST, ASSET_LABEL } from 'src/lib/pepefi/assetMeta'
...
export default function ExchangePage() {
  ...
  }, [contracts, wallet.address, wallet.provider, notify])
  ...
  // Clear loading when wallet not connected, or connected to an unsupported network
  const isWrongNetwork = wallet.isConnected && wallet.chainId !== null && !getAddresses(wallet.chainId)
  useEffect(() => {
    if (!wallet.isConnected || isWrongNetwork) setPageLoading(false)
  ...
  const selectedAssetPrice = livePrices[selAsset]?.usd
  useEffect(() => {
    if (selectedAssetPrice !== undefined) {
      setHistory(prev => {
        const next = [...prev, { time: new Date().toLocaleTimeString(), price: selectedAssetPrice }]
        return next.slice(-30) // last 30 data points (~1 min)
      })
    }
  }, [selectedAssetPrice, selAsset])
  ...
  const doSwap = async () => {
    ...
    try {
      if (swapMode === 'eth-to-usdc') {
        ...
        const swapAmt = parseEther(payAmount)
        ...
        if (currentAllowance < swapAmt) {
          notify('Approving mUSDC...', true)
          const approveTx = asTx(await contracts.usdc.approve(String(contracts.swapRouter.target), swapAmt))
          await approveTx.wait()
        }

        const routerBal = await wallet.provider!.getBalance(String(contracts.swapRouter.target))
        const expectedEth = swapAmt / 3000n
        ...
        const tx = asTx(await contracts.swapRouter.swapUSDCForETH(swapAmt))
        await tx.wait()
  ...
  const livePositions = positions.map(p => {
    ...
    const posNotional = p.margin * p.leverage
    const size = (posNotional * 10n**18n) / p.entryPrice
  ...
  if (isWrongNetwork) {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3">
        ...
        <p className="text-white font-semibold text-lg">Unsupported Network</p>
        <p className="text-gray-400 text-sm">
          Connected to <span className="text-warn font-mono">{name}</span>.
          Please switch to <span className="text-brand-300 font-mono">Sepolia</span> testnet.
        </p>
      </div>
    )
  }
  ...
  if (pageLoading) {
    return (
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-6 animate-pulse">
        <div className="h-24 bg-surface-elev rounded-card border border-surface-border" />
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="h-40 bg-surface-elev rounded-card border border-surface-border" />
          <div className="h-40 bg-surface-elev rounded-card border border-surface-border" />
        </div>
        <div className="h-64 bg-surface-elev rounded-card border border-surface-border" />
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6 fade-in relative">
          ...
          <div className="bg-[#131A2A] hover:border-gray-700 border border-transparent rounded-2xl p-4 transition-colors group">
            ...
            <div className="flex items-center gap-3">
              ...
              {swapMode === 'eth-to-usdc' ? (
                <button className="btn-secondary btn-pill btn-sm">
      ...
      <div className="rounded-card border border-surface-border bg-surface shadow-card p-5 space-y-5">
        ...
        <p className="text-xs text-gray-500 mt-2">
          ...
          <Link to="/pepefi/admin/oracle" className="text-emerald-400 hover:underline">Oracle Admin</Link> page.
        </p>
      ...
      <div className="rounded-card border border-surface-border bg-surface shadow-card p-5 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => void fetchAll()} className="btn-ghost">
            ↺ Refresh
          </button>
        </div>
